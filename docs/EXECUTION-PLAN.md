...
**M9 결과 (2026-09-22)**: 완료 기준 ①~④ 모두 충족. ①3장 자동 검증(`smoke:*` 전부, 1063개 테스트, 새 교차 플랫폼 DB 검사 포함) 통과. ②3.3 수동 체크리스트를 사용자가 실제 Windows·휴대폰에서 수행, 전부 정상(위 3.3 결과 참고 — 방화벽 규칙 문제 1건 발견·해결). ③요구사항 추적표 확정(4.4). ④`README.md` 작성(설치, 첫 실행, 단어장 다시 가져오기, 백업, 휴대폰 접속, 서버 옵션, 문제 해결), 방화벽 발견 사항 반영.
- **배포**: 사용자 승인으로 `npm run deploy`(`DEPLOY_DIR=/mnt/c/WordQuiz`)를 실행해 검증용으로 `C:\WordQuiz`에 배포했다(그 전까지 존재하지 않던 폴더). 이후 **사용자가 최종적으로 쓸 위치로 직접 zip을 옮겨** 설치했다(드라이브 무관 동작 확인 겸함).
- **발견한 문제**: 휴대폰 접속이 처음엔 안 됐다. 원인은 Windows 방화벽의 "Node.js JavaScript Runtime" 규칙이 **공용(Public)만 허용, 개인(Private) 미허용** 상태였던 것(집 Wi-Fi는 개인 네트워크). 개인 체크박스를 켜자 즉시(재시작 없이) 해결됐다. PC에 WSL·가상 어댑터가 있어 "From a phone..." 주소가 5줄 나왔는데, 그중 실제 Wi-Fi 어댑터의 IPv4와 일치하는 것을 쓰면 된다는 점도 확인했다. 둘 다 README에 반영했다.
- 이것으로 M0~M9 전 마일스톤이 끝났다. 남은 것은 이번 세션 상태 저장(`handoff`)과, 사용자가 원하면 커밋 push뿐이다.
...
### 3.3 수동 체크리스트 (사용자, Windows) — 완료 (2026-09-22)
- [x] `start.bat` 더블클릭 → 콘솔이 뜨고 기본 브라우저에 시작 화면이 열린다
- [x] 콘솔에서 `node -v`가 22.13 이상인지 확인, 미만이면 `start.bat`이 안내 메시지를 낸다
- [x] 이미 실행 중일 때 `start.bat`을 다시 실행하면 "이미 실행 중" 안내가 나온다
- [x] **콘솔 창을 X로 닫은 뒤** 다시 실행하면 이전 세션의 종료 시각이 기록되어 있다(`SIGHUP` 경로)
- [x] Ctrl+C로 종료해도 종료 시각이 기록된다
- [x] `import.bat`으로 실제 Excel을 검증 → 수정 → `--apply` 흐름이 되고 리포트가 메모장에서 깨지지 않는다
- [x] 같은 Wi-Fi의 휴대폰에서 `http://<PC 이름 또는 IP>:35000`으로 접속되고, 최초 방화벽 창에서 허용하면 동작한다
- [x] 휴대폰에서 입력 시 자동 고침·자동 대문자가 답을 바꾸지 않는다
- [x] PC와 휴대폰을 번갈아 써도 같은 세션이 이어진다

**결과**: 전부 정상. **한 가지 실제로 걸린 문제**: 휴대폰 접속이 처음엔 안 됐다 — Windows 방화벽의 "Node.js JavaScript Runtime" 규칙이 **공용(Public)만 체크되고 개인(Private)은 미체크** 상태였다(집 Wi-Fi는 개인 네트워크로 분류됨). "앱 허용" 설정에서 개인 체크박스를 켜자 즉시 해결됐다(재시작 불필요). 이 내용은 README "문제 해결"·"휴대폰에서 접속하기"에 반영했다. 사용자는 산출물을 `C:\WordQuiz`가 아닌 **자신이 고른 드라이브**(zip을 그대로 옮겨 폴더 생성)에 배포해 검증했다 — 드라이브 무관 동작도 함께 확인된 셈이다.
